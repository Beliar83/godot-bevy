...
impl AnimationLibrarySignals {
    /// Emitted when an  is added, under the key `anim_name`.
    pub const ANIMATION_ADDED: &'static str = "animation_added";

    /// Emitted when an  stored with the key `anim_name` is removed.
    pub const ANIMATION_REMOVED: &'static str = "animation_removed";

    /// Emitted when the key for an  is changed, from `old_name` to `new_name`.
...
impl AnimationNodeSignals {
    ...
    /// Emitted by  when its `AnimationNodeAnimation.animation` resource is changed, or by  when its connections change.
    pub const NODE_UPDATED: &'static str = "node_updated";
...
impl Area3DSignals {
    /// Emitted when a  of the received `body` enters a shape of this area. `body` can be a ,  or . s are detected if their  has collision shapes configured. Requires `monitoring` to be set to `true`.
    /// `local_shape_index` and `body_shape_index` contain indices of the interacting shapes from this area and the interacting body, respectively. `body_rid` contains the  of the body. These values can be used with the .
    /// **Note:** Godot Physics does not support reporting overlaps with , so will not emit this signal in such cases.
    /// **Example:** Get the  node from the shape index:
    ///
    /// ```gdscript
    ///
    ...
    /// Emitted when a  of the received `body` exits a shape of this area. `body` can be a ,  or . s are detected if their  has collision shapes configured. Requires `monitoring` to be set to `true`.
    /// See also `body_shape_entered`.
    /// **Note:** Godot Physics does not support reporting overlaps with , so will not emit this signal in such cases.
    pub const BODY_SHAPE_EXITED: &'static str = "body_shape_exited";

    /// Emitted when the received `body` enters this area. `body` can be a ,  or . s are detected if their  has collision shapes configured. Requires `monitoring` to be set to `true`.
    /// **Note:** Godot Physics does not support reporting overlaps with , so will not emit this signal in such cases.
    pub const BODY_ENTERED: &'static str = "body_entered";

    /// Emitted when the received `body` exits this area. `body` can be a ,  or . s are detected if their  has collision shapes configured. Requires `monitoring` to be set to `true`.
    /// **Note:** Godot Physics does not support reporting overlaps with , so will not emit this signal in such cases.
    pub const BODY_EXITED: &'static str = "body_exited";
...
impl ControlSignals {
    ...
    pub const MAXIMUM_SIZE_CHANGED: &'static str = "maximum_size_changed";
...
/// Signal constants for `DisplayServer`
pub struct DisplayServerSignals;

impl DisplayServerSignals {
    ...
    pub const ORIENTATION_CHANGED: &'static str = "orientation_changed";
}
...
/// Signal constants for `InputMap`
pub struct InputMapSignals;

impl InputMapSignals {
    /// Emitted when the   has been loaded.
    pub const PROJECT_SETTINGS_LOADED: &'static str = "project_settings_loaded";
}
...
impl OpenXrInterfaceSignals {
    ...
    pub const USER_PRESENCE_CHANGED: &'static str = "user_presence_changed";
}
...
impl OpenXrSpatialEntityTrackerSignals {
    ...
    pub const NEXT_CHANGED: &'static str = "next_changed";
...
/// Signal constants for `VirtualJoystick`
pub struct VirtualJoystickSignals;

impl VirtualJoystickSignals {
    ...
    pub const FLICKED: &'static str = "flicked";

    /// Emitted when the tip enters the deadzone after being outside of it.
    pub const FLICK_CANCELED: &'static str = "flick_canceled";
}
...
impl WindowSignals {
    ...
    pub const OUTPUT_MAX_LINEAR_VALUE_CHANGED: &'static str = "output_max_linear_value_changed";
}
...
impl XrServerSignals {
    ...
    pub const WORLD_ORIGIN_CHANGED: &'static str = "world_origin_changed";
}
